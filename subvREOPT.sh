...
# Execute the program
cp -v /home/mewes/bin/vasp_tools/vdw_kernel.bindat \$SLURM_SUBMIT_DIR
cd \$SLURM_SUBMIT_DIR
...
echo "---- First opt finished at \$(date) ----"

#How many steps to converge?
nsteps=\$(egrep "^   [1-9] F=" output | awk '{print \$1}' | tail -n 1)
nopt=0

touch first_cycle_converged_in_\${nsteps}_steps
cp POSCAR POSCAR.start

#Repeat optimization until converged in one step
while [ \$nsteps -gt 1 ] && [ \$nopt -lt 18 ] ; do
  mv output output_\$nopt
  cp CONTCAR POSCAR
  rm *_of_optimization__used_*
  srun -n $actual_ncores $BINARY > output
  nsteps=\$(egrep "^   [1-9] F=" output | awk '{print \$1}' | tail -n 1)
  echo "Took \$nsteps to converge..."
  nopt=\$((nopt+1))
  touch meta_cycle_\$((nopt+1))__converged_in_\${nsteps}_steps
done

rm  *bindat
touch optimization_concluded_after_\$((nopt+1))_meta_cycles
